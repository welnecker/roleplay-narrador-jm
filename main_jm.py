...
def mj_set_fase(n: int, persist: bool=False):
    """
    Ajusta a fase do romance apenas em sessão (persist=False ignora qualquer gravação).
    """
    try:
        n = int(n)
    except Exception:
        n = 0
    n = max(0, min(max(FASES_ROMANCE.keys()), n))
    st.session_state.mj_fase = n

def mj_carregar_fase_inicial() -> int:
    """
    Carrega a fase da sessão (sem persistência em planilha).
    """
    return int(st.session_state.get("mj_fase", 0))

def momento_set(n: int, persist: bool = False):
    """
    Ajusta o momento apenas em sessão.
    """
    try:
        n = int(n)
    except Exception:
        n = 0
    n = max(0, min(max(MOMENTOS.keys()), n))
    st.session_state.momento = n

def momento_carregar() -> int:
    """
    Carrega o momento da sessão (sem persistência).
    """
    return int(st.session_state.get("momento", 0))
...
def _momento_label(n: int) -> str:
    d = MOMENTOS.get(int(n), MOMENTOS[0])
    return f"{int(n)} — {d['nome']}"
...
# Histórico do Sheets
    n_hist = int(st.session_state.get("n_sheet_prompt", 10))
    ...
    # Apoio semântico local (não usa Sheets)
    try:
        corpus = _corpus_cena(memos, hist, ate_ts)
        sem_topk = busca_semantica_local(hist[-1]["content"] if hist else "", corpus, k=5, limiar=0.35)
        sem_txt = "\n".join(f"- {t}" for (t, _s) in sem_topk) if sem_topk else "(nenhuma)"
    except Exception:
        sem_txt = "(nenhuma)"
