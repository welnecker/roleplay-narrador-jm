...
FASES_ROMANCE: Dict[int, Dict[str, str]] = {
    ...
    5: {"nome": "Compromisso / Encontro definitivo",
        ...
        # Alteração: Permitir interações explícitas neste estágio, removendo proibições

        "permitidos": "beijo prolongado; dormir juntos; consumação explícita e detalhada permitida",
...
MOMENTOS = {
    ...
    3: {"nome": "Consentimento explícito",
        ...
        "permitidos": "nomear fronteiras; pedir/receber consentimento; decidir 'agora sim'; descrição gráfica permitida",
    ...
    4: {"nome": "Intimidade (elíptica)",
        ...
        "objetivo": "intimidade livre e explícita",

        "permitidos": "beijos longos; proximidade forte; descrição gráfica de atos sexuais e anatômicos",
...
def detectar_momento_sugerido(texto: str, fallback: int = 0) -> int:
    ...
    # Removida a verificação que limitava momentos por palavras explícitas para permitir conteúdos explícitos
...
def viola_momento(texto: str, momento: int) -> str:
    ...
    # Removido bloqueio que ignorava descrições explícitas para permitir interações explícitas
...
""".strip()
    ...
    if fase >= 5:
        ...
        # Removida a restrição de corte elíptico: permite descrição completa e explícita

        regras += """

- Intimidade com detalhamento explícito permitido."""
...
with st.sidebar:
    ...
    st.checkbox("Usar memória longa no prompt",
                ...
                value=st.session_state.get("use_memoria_longa", False),  # Desativado por padrão para evitar filtro
    ...
    st.checkbox("Ativar filtro", value=False, key="nsfw_filter_on")  # Desativado por padrão

    st.slider("Limite de calor (0=safe · 1=sensual · 2=forte · 3=explícito)", 0, 3, value=3, key="nsfw_max_level")
...
def render_tail(t: str) -> str:
    ...
    # remove rótulos meta (Microconquista/Gancho) e blocks
...
def classify_nsfw_level(t: str) -> int:
    ...
    # Mantém classificação, mas não interfere na sanitização
...
def sanitize_explicit(t: str, max_level: int, action: str) -> str:
    ...
    # Libera texto sem sanitização para permitir conteúdo explícito
...
if entrada:
    ...
    with st.chat_message("assistant"):
        ...
        # 5) Verificação de Momento e reescrita removida para permitir completamente conteúdo explícito
